  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        {/* Package contents — fixed: QRE spreadsheet + supporting docs */}
        <div
          style={{
            background: 'rgba(0,0,0,0.2)',
            border: '1px solid rgba(240,231,215,0.1)',
            borderRadius: 4,
            padding: '16px 18px',
            marginBottom: 20,
          }}
        >
          <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '2px', textTransform: 'uppercase', color: 'rgba(240,231,215,0.5)', marginBottom: 10 }}>
            Package Contents
          </div>
          <div style={{ fontSize: 12, color: 'var(--ivory)', fontWeight: 400, marginBottom: 6 }}>
            <span style={{ marginRight: 8 }}>📊</span>
            QRE Spreadsheet (Excel) — partner-facing template
          </div>
          <div style={{ fontSize: 12, color: documentCount > 0 ? 'var(--ivory)' : 'rgba(240,231,215,0.4)', fontWeight: 400 }}>
            <span style={{ marginRight: 8 }}>📁</span>
            Supporting Documents (ZIP){documentCount === 0 ? ' — no documents uploaded' : ` — ${documentCount} file${documentCount !== 1 ? 's' : ''}`}
          </div>
        </div>
        ...
        {result && (
          ...
          >
            ...
            <div className="flex flex-wrap" style={{ gap: 8 }}>
              {result.excel_url && (
                <a href={result.excel_url} target="_blank" rel="noopener noreferrer" style={linkStyle}>📊 QRE Spreadsheet</a>
              )}
